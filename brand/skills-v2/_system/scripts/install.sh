...
#
# Usage:
#   ./install.sh              Install to ~/.claude/skills/ (Claude Code + Cursor)
#   ./install.sh --cursor     Install to ~/.cursor/skills/ (Cursor-only)
#   ./install.sh --claude-only  Install only Claude-native skills (no Replicate)
#
# Target platforms:
#   Default (~/.claude/skills/): Works for Claude Code, Claude Code Desktop,
#     Terminal, AND Cursor (Cursor reads ~/.claude/skills/ for compatibility)
#   --cursor (~/.cursor/skills/): Cursor-specific install
#
# Environment:
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --cursor)
      INSTALL_CURSOR=true
      INSTALL_HOME="$HOME/.cursor"
      SKILLS_DIR="$INSTALL_HOME/skills"
      SYSTEM_DIR="$SKILLS_DIR/_system"
      shift
      ;;
    --help|-h)
      echo "Usage: $0 [--claude-only] [--cursor]"
      echo ""
      echo "Options:"
      echo "  --claude-only  Skip the creative skill (requires Replicate API)"
      echo "  --cursor       Install to ~/.cursor/skills/ (Cursor-only)"
      echo "                 Default: ~/.claude/skills/ (Claude Code + Cursor)"
...
if $INSTALL_CURSOR; then
  info "Mode:    Cursor (~/.cursor/skills/)"
elif $CLAUDE_ONLY; then
  warn "Mode:    Claude-only (creative skill excluded)"
else
  info "Mode:    Full suite (Claude Code + Cursor)"
fi
echo ""
